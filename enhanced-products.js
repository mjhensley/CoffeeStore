...
(function() {
    ...
    // Constants
    const SUBSCRIPTION_DISCOUNT = 0.10; // 10% off
    const SIZE_CONFIG = {
        '12oz': { label: '12 oz', multiplier: 1.0, weight: '340g' },
        '2lb': { label: '2 lb', multiplier: 2.5, weight: '907g', discount: 0.06 },
        '5lb': { label: '5 lb', multiplier: 5.8, weight: '2.27kg', discount: 0.10 }
    };
    ...
    function calculatePrice(basePrice, size, isSubscription) {
        ...
        // Apply bulk discount for larger sizes
        if (sizeConfig.discount) {
            price = price * (1 - sizeConfig.discount);
        }
    ...
    function updateButtonData(button, state, price) {
        ...
        if (button.classList.contains('snipcart-add-item')) {
            ...
            button.setAttribute('data-item-custom1-value', state.selectedSize);
